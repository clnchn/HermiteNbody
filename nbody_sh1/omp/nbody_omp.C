...
int main(int argc, char *argv[])
{
    ...
    real  dt_tot = 1000000;       // duration of the integration
...
//instead of array mass[], we take the mass of one body to calculate
void evolve(const real mass[], real pos[][NDIM], real vel[][NDIM],
            int n, real & t, real dt_param, real dt_dia, real dt_out,
            real dt_tot, bool init_out, bool x_flag)
{
    ...
    cout << "Starting a Hermite integration for a " << n
         << "-body system,\n  from time t = " << t
         << " with time step control parameter dt_param = " << dt_param
         << "  until time " << t + dt_tot
         << " ,\n  with diagnostics output interval dt_dia = "
    ...
    //#pragma omp parallel private(t_dia, t_out, t_end) shared(t, n)
    //#pragma omp parallel
    
    while (true){
        while (t < t_dia && t < t_out && t < t_end){
            real dt = dt_param * coll_time;
            evolve_step(mass, pos, vel, acc, jerk, n, t, dt, epot, coll_time); //parallel this step
            nsteps++;
        }
                 ...
                 real dt, real & epot, real & coll_time)
{
    ...
    for (int i = 0; i < n ; i++){
        for (int k = 0; k < NDIM ; k++){
            predict_step(pos[i][k], velpos[i][k], accpos[i][k], jerkpos[i][k], n, dt);
            get_acc_jerk_pot_coll(mass[i], pos[i][k], vel[i][k], acc[i][k], jerk[i][k], n, epot, coll_time);
            correct_step(pos[i][k], vel[i][k], acc[i][k], jerk[i][k], old_pos[i][k], old_vel[i][k], old_acc[i][k], old_jerk[i][k], n, dt);
        }
    }
    */
    
    
    //Parallel this at one single time
...
//{
    ...
    //for (int i = 0; i < n ; i++){
        //cerr <<  "openMP =  " << omp_get_num_threads() << endl;
        //for (int k = 0; k < NDIM ; k++){
...
//celene
void predict_step(real pos, real vel,
                  const real acc, const real jerk,
                  int n, real dt)
{
    pos += vel*dt + acc*dt*dt/2 + jerk*dt*dt*dt/6;
    vel += acc*dt + jerk*dt*dt/2;
