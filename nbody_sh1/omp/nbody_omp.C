...
{
    for (int k = 0; k < NDIM ; k++){
        pos[bodyID][k] += vel[bodyID][k]*dt + acc[bodyID][k]*dt*dt/2
                                  + jerk[bodyID][k]*dt*dt*dt/6;
        vel[bodyID][k] += acc[bodyID][k]*dt + jerk[bodyID][k]*dt*dt/2;
    }
}
...
{
    for (int k = 0; k < NDIM ; k++){
        vel[bodyID][k] = old_vel[bodyID][k] + (old_acc[bodyID][k] + acc[bodyID][k])*dt/2
                                  + (old_jerk[bodyID][k] - jerk[bodyID][k])*dt*dt/12;
        pos[bodyID][k] = old_pos[bodyID][k] + (old_vel[bodyID][k] + vel[bodyID][k])*dt/2
                                  + (old_acc[bodyID][k] - acc[bodyID][k])*dt*dt/12;
    }
}
    ...
    for (int j = 0; j < n ; j++){
        if(j == bodyID){
            continue;
                                                ...
                                                // rji[] is the vector from
        real rji[NDIM];                        // particle i to particle j
        real vji[NDIM];                        // vji[] = d rji[] / d t
        for (int k = 0; k < NDIM ; k++){
            rji[k] = pos[j][k] - pos[bodyID][k];
            vji[k] = vel[j][k] - vel[bodyID][k];
        }
        real r2 = 0;                           // | rji |^2
        real v2 = 0;                           // | vji |^2
...
// add the {j (i)} contribution to the {i (j)} values of acceleration and jerk:
        ...
        for (int k = 0; k < NDIM ; k++){
            acc[bodyID][k] += mass[j] * da[k];                 // using symmetry
            acc[j][k] -= mass[bodyID] * da[k];                 // find pairwise
            jerk[bodyID][k] += mass[j] * dj[k];                // acceleration
            jerk[j][k] -= mass[bodyID] * dj[k];                // and jerk
        }
...
// second collision time estimate, based on free fall:
        ...
        double mij = mass[bodyID] + mass[j];                // eration between
